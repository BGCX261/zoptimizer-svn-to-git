...
read(buffer[, offset[, nbytes[, flags]]]) -- read data (into a buffer with offset)\n\
recvfrom(buflen[, flags]) -- receive data and sender\'s address\n\
recvfrom_into(buffer[, nbytes, [, flags])\n\
  -- receive data and sender\'s address (into a buffer)\n\
sendall(data[, flags]) -- send all data\n\
send(data[, flags]) -- send data, may not send all of it\n\
write(buffer[, offset[, nbytes[, flags]]]) -- write data from buffer, may not send all of it\n\
...
/*
 * Modified by Jacky Wang (chaowang@zoptimizer.com)
 * call if the offset of this buffer is specified.
 */
static ssize_t
sock_recv_guts2(PySocketSockObject *s, char* cbuf, int offset, int len, int flags)
{
    return sock_recv_guts(s, cbuf + offset, len, flags);
}
...
/* s.read(buffer[, offset[, nbytes [,flags]]]) method */

static PyObject*
sock_read(PySocketSockObject *s, PyObject *args, PyObject *kwds)
{
    static char *kwlist[] = {"buffer", "offset", "nbytes", "flags", 0};

    int offset = 0, recvlen = 0, flags = 0;
    ssize_t readlen;
    Py_buffer buf;
    Py_ssize_t buflen;

    /* Get the buffer's memory */
    if (!PyArg_ParseTupleAndKeywords(args, kwds, "w*|iii:read", kwlist,
                                     &buf, &offset, &recvlen, &flags))
        return NULL;
    buflen = buf.len;
    assert(buf.buf != 0 && buflen > 0);
    ...
    /* Call the guts */
    readlen = sock_recv_guts2(s, buf.buf, offset, recvlen, flags);
    if (readlen < 0) {
        /* Return an error. */
        goto error;
    }
...
"read(buffer[, offset[, nbytes [,flags]]]) -> nbytes_read\n\
\n\
A new version of recv() that stores its data into a buffer rather than creating \n\
a new string.  Receive up to buffersize bytes from the socket.  If buffersize \n\
is not specified (or 0), receive up to the size available in the given buffer.\n\
\n\
See recv() for documentation about the flags.");
...
/* s.write(buffer[, offset[, nbytes[, flags]]]) method */

static PyObject *
sock_write(PySocketSockObject *s, PyObject *args, PyObject *kwds)
{
    static char *kwlist[] = {"buffer", "offset", "nbytes", "flags", 0};

    int offset = 0, sendlen = 0, flags = 0, n = -1, timeout;
    char *buf;
    Py_buffer pbuf;

    /* Get the buffer's memory */
    if (!PyArg_ParseTupleAndKeywords(args, kwds, "w*|iii:write", kwlist,
                                     &pbuf, &offset, &sendlen, &flags))
        return NULL;
    buf = pbuf.buf;
    assert(buf != 0 && pbuf.len > 0);

    Py_BEGIN_ALLOW_THREADS
    timeout = internal_select(s, 1);
    if (!timeout)
#ifdef __VMS
        n = sendsegmented(s->sock_fd, buf, sendlen, flags);
#else
        n = send(s->sock_fd, buf, sendlen, flags);
#endif
    Py_END_ALLOW_THREADS
...
"write(buffer[, offset[, nbytes[, flags]]]) -> count\n\
\n\
Send a buffer with offset to the socket.  For the optional flags\n\
argument, see the Unix manual.  Return the number of bytes\n\
sent; this may be less than nbytes if the network is busy.");
...
static PyMethodDef sock_methods[] = {
    ...
    {"recv_into",         (PyCFunction)sock_recv_into, METH_VARARGS | METH_KEYWORDS,
                      recv_into_doc},
    {"read",              (PyCFunction)sock_read, METH_VARARGS | METH_KEYWORDS,
                      read_doc},
    ...
    {"send",              (PyCFunction)sock_send, METH_VARARGS,
                      send_doc},
    {"write",             (PyCFunction)sock_write, METH_VARARGS | METH_KEYWORDS,
                      write_doc},
